...
<div id="HomeContainer" class="row home-container"
    ...
    ?>
        ...
        if(!empty($recentlyvisited)){ 
            ...
            $universeIds = array_column((array)$recentlyvisited, 'universeid');

            $universesRaw = $db->table("universes")
                ->whereIn("assetid", $universeIds)
                ->limit(6)
                ->get();

            $universes = [];
            foreach ($universesRaw as $u) {
                $universes[$u->assetid] = $u;
            }
        ...
        <div id="recently-visited-places" class="col-xs-12 container-list home-games">
            ...
            <?  foreach($recentlyvisited as $visit){
                ...
                $universeinfo = $universes[$visit->universeid] ?? null;
        ...
        <div id="recently-visited-places" class="col-xs-12 container-list home-games">
            ...
            $randomlypicked = $db->table("universes")
                ->whereNot("title", "LIKE", "%Place")
                ->whereNot("title", "LIKE", "%Place Number:%")
                ->orderBy($db->raw("RAND()"))
                ->limit(6)
                ->get();

            foreach($randomlypicked as $game){
